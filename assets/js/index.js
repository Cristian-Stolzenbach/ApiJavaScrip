...
searchButton.addEventListener("click", () => {
  ...
  if (coin == "dolar") {
    ...
    function prepareChartConfig(dolares) {
      ...
      const title = "Dolar ultimos 10 dias";
      const lineColor = "red";
    ...
    function prepareChartConfig(euros) {
      ...
      const title = "Euro ultimos 10 dias";
    ...
    function prepareBitcoinChartConfig(bitcoins) {
      const graphType = "line";
      const dateBitcoin = bitcoins.serie
        .slice(0, 10)
        .map((bitcoin) => bitcoin.fecha);
      const title = "Bitcoin ultimos 10 dias";
      const lineColor = "green";
      const pointColor = "white";
      const values = bitcoins.serie.map((bitcoin) => {
        const value = bitcoin.valor;
        return Number(value);
      });
    ...
    async function renderBitcoinGraph() {
      const bitcoins = await getBitcoin();
      const config = prepareBitcoinChartConfig(bitcoins);
      const chartDOM = document.getElementById("myChart");
      myChart = new Chart(chartDOM, config);
    }

    async function renderBitcoinResult() {
      const bitcoins = await getBitcoin();
      const amountToConvert = inputValue.value;
